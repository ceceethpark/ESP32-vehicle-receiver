...
void     LCD_IO_WriteCmd8DataFill16_MSB(uint8_t Cmd, uint16_t Data, uint32_t Size);
...
//#define LCD_DATA16_WRITE_MSB(d16)   {LCD_DATA8_WRITE(d16 >> 8); LCD_DATA8_WRITE(d16); }
...
//#define LCD_DATA8_WRITE(dt)   { lcd_data8bit = dt; LCD_WRITE(lcd_data8bit); GPIOX_ODR(LCD_WR) = 0; LCD_WR_DELAY; GPIOX_ODR(LCD_WR) = 1; }

void LCD_DATA8_WRITE(uint8_t dt)   {
	uint8_t lcd_data8bit = dt;
	LCD_WRITE(lcd_data8bit);
	GPIOX_ODR(LCD_WR) = 0;
	LCD_WR_DELAY;
	GPIOX_ODR(LCD_WR) = 1;
}
...
//                   GPIOX_CLOCK_LCD_RST | GPIOX_CLOCK_LCD_BL  | GPIOX_CLOCK_LCD_RD);
  ...
  RCC->APB2ENR |= (GPIOX_CLOCK(LCD_CS) | GPIOX_CLOCK(LCD_RS) | GPIOX_CLOCK(LCD_WR) |
                   GPIOX_CLOCK(LCD_D0) | GPIOX_CLOCK(LCD_D1) | GPIOX_CLOCK(LCD_D2) | GPIOX_CLOCK(LCD_D3) |
                   GPIOX_CLOCK(LCD_D4) | GPIOX_CLOCK(LCD_D5) | GPIOX_CLOCK(LCD_D6) | GPIOX_CLOCK(LCD_D7) |
                   GPIOX_CLOCK_LCD_RST | GPIOX_CLOCK_LCD_BL );
  #if GPIOX_PORTNUM(LCD_RST) >= GPIOX_PORTNUM_A
  LCD_RST_OFF;                          // RST = 1
...
//  uint8_t CE=Data &0xFF;
//  LCD_CS_ON;
//  LCD_CMD8_WRITE(Cmd);
// // HI=0x55;
// // CE=0xFF;
// // LO=0x55;
//  printf("LCD_IO_WriteCmd8DataFill16_MSB [%02x]-->HI[%02x] LO[%02x]\r\n", Cmd, HI, LO);
//  while(Size--)
//  {
//    //LCD_DATA16_WRITE(Data);
//   // LCD_DATA16_WRITE_MSB(Data);
//
//    //LCD_DATA8_WRITE(CE);
//     LCD_DATA8_WRITE(LO);
//     LCD_DATA8_WRITE(HI);
